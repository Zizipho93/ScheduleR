...
function loadButtonEvents(){
    ...
    var submitBackButton = document.getElementById('submit-back')
...
function loadmap(){
    mapboxgl.accessToken = 'pk.eyJ1IjoibWtvbnR3YW5hIiwiYSI6ImNqbXVieTg0ejJ2YzgzcHBkdTkxMDI4cmgifQ.zivgkLXhZ2LluwKmpBRP7w';
    ...
    window.destinationtPin = new mapboxgl.Marker().setLngLat([0, 0]).addTo(window.map)

    window.map.on('click' , function (event){
        console.log(event)
        if(window.startPoint == true){
            window.destinationtPin.setLngLat(event.lngLat)
